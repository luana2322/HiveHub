...
function send(event) {
  ...
  if (messageContent && stompClient) {
    var chatMessage = {
      ...
      receiverName: 'Hello everyone',
      date: new Date().toISOString(),
      type: 'CHAT',
      user_project_id: 11, // Thay thế bằng ID thực tế của dự án người dùng
      userId: 1
    };
...
function onMessageReceived(payload) {
  ...
  if (message.type === "JOIN") {
    ...
    // * update
    usernameElement.style["color"] = getAvatarColor(message.senderName);
    //* update end
  }
